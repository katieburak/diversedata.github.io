...
We can also see histograms for all our different categorical variables to see how different groups were sampled in the survey. This way we can note that the survey considered:
...
-   Subjects mostly agreed about their approach to the pandemic with their partner, including being fully vaccinated.
...
In order to do so, we need to make sure that our response variable is encoded as an ordered factor. We will also take this opportunity to encode all our explanatory variables as factors (and ordered factors) as needed.
...
For our modeling purposes, we will use the `MASS` package, `polr()` function to obtain model estimates obtained through maximum likelihood.

Given that we are using ordinal categorical variables as explanatory variables, we will also set our model to use successive differences contrasts to make our interpretation of the estimates more interpretable.
...
```{r}
partial_ordinal_model <- polr(relationship_quality ~ sex_frequency + flirts_with_partner + fights_with_partner,
  data = hcmst, Hess = TRUE
)
