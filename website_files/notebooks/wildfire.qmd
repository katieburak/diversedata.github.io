...
```{r load_libraries_geospatial_analysis, results = "hide", warning = FALSE, message = FALSE}
library(tidyverse)         
library(gt)
library(ggmap)
library(ggspatial)
...
```{r prepare_wildfire_data_for_modeling, warning=FALSE, message=FALSE, echo=FALSE}
...
```{r prepare_wildfire_map_data, warning=FALSE, message=FALSE}
...
```{r plot_wildfire_size_weighted_heatmap, warning=FALSE, message=FALSE, fig.width=7, fig.height=9}
# Plot heatmap
ggplot() +
...
```{r save_wildfire_heatmap_plot, warning=FALSE, message=FALSE, echo=FALSE}
heatmap_plot <- 
ggplot() +
  layer_spatial(basemap) +
  stat_density_2d_filled(
...
```{r plot_proportion_large_fires_by_cause}
wildfire_clean |>
  group_by(true_cause) |>
...
```{r logistic_model_large_fires_summary, warning=FALSE}
...
```{r plot_marginal_effects_wind_by_cause}
p <- plot_comparisons(
  model,
...
```{r plot_marginal_effects_humidity_by_fire_type}
