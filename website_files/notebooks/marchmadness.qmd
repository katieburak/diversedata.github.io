...
This data set contains historical records of every NCAA Division I Women’s Basketball Tournament appearance since the tournament began in 1982. Sourced from official NCAA data, it captures team-level performance and tournament outcomes across more than four decades of collegiate women’s basketball.
...
-   **Home Game Indicator** – Whether the team played its first game at home

### Purpose and Use Cases

This dataset is designed to support analysis of:

-   Team success and consistency over time

-   Impact of seeding and bid types on tournament outcomes

-   Conference strength and historical performance trends

-   Emergence and decline of dynasties in women’s college basketball
...
This analysis explores the relationship between a team’s `seed` and its tournament performance, as measured by tournament wins, to evaluate whether higher-seeded teams consistently outperform lower-seeded ones.

By examining over four decades of historical data, we aim to:

-   Identify trends in tournament advancement by seed level

Seeding is intended to reflect a team’s regular-season performance and overall strength. In theory, lower-numbered seeds (e.g., #1, #2) are given to the strongest teams, who should be more likely to advance. But upsets, bracket surprises, and standout performances from lower seeds raise questions like "How reliable is seeding as a predictor of success?"

Understanding these dynamics can inform not just fan expectations and bracket predictions.
...
First, we can see which seeds appear more often.
...
Remember, Poisson assumes that the mean is equal to the variance of the count variable. If the variance is much greater, we might need a Negative Binomial model. We can do an dispersion test to evaluate this matter.

Let $Y_i$ be the $ith$ Poisson response in the count regression model. In the presence of equidispersion, $Y_i$ has the following parameters:
...
3.  **Variability remains**: While seeding generally reflects team strength, upsets and unexpected performances do occur, showing that other factors also influence tournament outcomes.

Seeding is an important predictor of success, but it is not foolproof. It sets expectations, but unexpected performances still shape the narrative of March Madness. Understanding this relationship can inform future discussions on fairness and tournament structure.